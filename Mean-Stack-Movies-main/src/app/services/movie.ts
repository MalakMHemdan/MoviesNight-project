...
export class MovieService {
  ...
  getTrendingMovies(page: number = 1): Observable<any> {
    ...
    return this.http.get(`${this.baseUrl}/trending/movie/week?page=${page}`, {
      headers,
    });
  }
  ...
  getRecommendedMovies(): Observable<any> {
    ...
    return this.http.get(`${this.baseUrl}/trending/movie/week?page=1`, {
      headers,
    });
  }

  // دوال جديدة مضافة من غير تعديل القديم
  getMovieDetails(id: string): Observable<any> {
    const headers = new HttpHeaders({
      Authorization: `Bearer ${this.accessToken}`,
    });
    return this.http.get(`${this.baseUrl}/movie/${id}`, { headers });
  }
